...
SYSTEM_PROMPT = (
    ...
    "3) Если выбранный pattern оканчивается на ':', обязательно укажи поле docstring у шага.\n"
    "4) Для SQL-шагов вида 'Выполнить запрос в базу ...' также обязательно передавай SQL в docstring, даже если в pattern нет ':'.\n"
    "5) docstring передавай как обычную JSON-строку с переносами \\n (без тройных кавычек в самом JSON).\n"
    "6) Никаких новых шагов, только найденные через tool.\n"
    "7) Язык названий feature/scenario - русский.\n"
    "8) При выборе шага из результатов search_steps, обязательно извлеки из pattern все плейсхолдеры и заполни их в params на основе контекста ручного теста."
)
