...
import { buildChatMessage, buildResetMessage } from './protocol.js';
...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    private logBuffer: Array<{ level: string; text: string }> = [];

    constructor(
        private readonly extensionUri: vscode.Uri,
        private readonly manager: PythonManager,
    ) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case 'sendPrompt': {
                    ...
                    const featureFilePath = this.resolveFeaturePath(
                        cfg.get<string>('featureFilePath', '')
                    );
                    this.manager.sendLine(
                        buildChatMessage(msg.prompt, {
                ...
                case 'resetThread':
                    this.manager.sendLine(buildResetMessage());
                    break;
    ...
    private sendInitSettings(): void {
        const cfg = vscode.workspace.getConfiguration('spydr');
        this.post({
            type: 'initSettings',
            openaiApiKey: cfg.get<string>('openaiApiKey', ''),
            connectionString: cfg.get<string>('connectionString', ''),
            featureFilePath: cfg.get<string>('featureFilePath', ''),
            projectId: cfg.get<string>('projectId', ''),
            llmModel: cfg.get<string>('llmModel', 'gpt-4.1-mini'),
        });
    }

    private resolveFeaturePath(raw: string): string {
        if (!raw.trim()) { return ''; }
        if (path.isAbsolute(raw)) { return raw; }
        const folders = vscode.workspace.workspaceFolders;
        if (!folders?.length) { return raw; }
